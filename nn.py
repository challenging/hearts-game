...
class PolicyValueNet(object):
    def __init__(self, model_file=None):
        ...
        self.cards = tf.placeholder(tf.int32, shape=[None, 13], name="valid_cards")
        self.probs = tf.placeholder(tf.float32, shape=[None, 13], name="mcts_probs")
        ...
        input = tf.reshape(tf.multiply(cards_flat, self.states), [-1, 13*16*9])
        input1 = tf.layers.dense(input, units=128, activation=tf.nn.relu)
        input2 = tf.layers.dense(input1, units=64, activation=tf.nn.relu)

        # 3. Policy Networks
        self.action_fc = tf.layers.dense(inputs=input2, units=13, activation=tf.nn.softmax)
